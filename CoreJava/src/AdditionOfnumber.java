...
public class AdditionOfnumber
{
	 ...
	 // Time complexity: O(n)
		/*
		 * private static int[] findTwoSum(int[] nums, int target) { Map<Integer,
		 * Integer> numMap = new HashMap<>(); for (int i = 0; i < nums.length; i++) {
		 * int complement = target - nums[i];
		 * 
		 * if (numMap.containsKey(complement)) {
		 * System.out.println(complement+" :: "+nums[i]);
		 * 
		 * } else { numMap.put(nums[i], i); //numMap.put(complement,i); } } return new
		 * int[] {}; }
		 */
